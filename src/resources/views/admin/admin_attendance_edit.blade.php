        ...
        breakList.addEventListener('click', (e) => {
            if (e.target.classList.contains('delete-break-btn')) {
                ...
                breakList.querySelectorAll('.break-item').forEach((el, i) => {
                    el.dataset.index = i;
                    el.querySelector('input[name^="breaks"]').name = `breaks[${i}][start_time]`;
                    el.querySelectorAll('input')[1].name = `breaks[${i}][end_time]`;
                    el.querySelector('button') ? el.querySelector('button').textContent = '削除' : null;
